...
class PublicationSelector
{
    ...
    public function getPlayerPublications(): array
    {
        if (!isset($this->player_publications)) {
            ...
            $player_sub_usages = $this->publication_sub_usage_repository->convertSubsToUsage(PublicationUsage::USAGE_PLAYER);
            $player_usages = array_merge([$player_usage], $player_sub_usages);
            $internal_video_player = PluginConfig::getConfig(PluginConfig::F_INTERNAL_VIDEO_PLAYER);
            $player_usages = array_map(function ($usage) use ($internal_video_player) {
                if ($internal_video_player) {  // force media for internal player
                    $usage->setMdType(PublicationUsage::MD_TYPE_MEDIA);
                }
                return $usage;
            }, $player_usages);
            foreach ($player_usages as $player_usage) {
                $usage_pubs = $this->getPublicationMetadataForUsage($player_usage);
                if (!empty($usage_pubs)) {
